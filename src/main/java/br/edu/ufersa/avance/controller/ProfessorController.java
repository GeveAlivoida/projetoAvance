...
public class ProfessorController {
    ...
    private void configurarTabela(){
        ...
        colConta.setCellValueFactory(new PropertyValueFactory<>("contaBanco"));
        ...
        statusField.setConverter(new StringConverter<StatusProfessor>() {
            ...
            public StatusProfessor fromString(String string) {
                ...
                for (StatusProfessor status : StatusProfessor.values()) {
                    if (status.getDescricao().equalsIgnoreCase(string)) {
                        return status;
                    }
                }
                return null;
    ...
    private void configurarFormatacaoSalario(){
    ...
    void cadastrarProfessor(ActionEvent event) {
        try{
            ...
            novoProfessor.setStatus(statusField.getValue());
            ...
            mostrarMensagem(erroCadastro, "Salário inválido! Use o formato 0,00", Color.WHITE);
        }catch (IllegalArgumentException e) {
            mostrarMensagem(erroCadastro, e.getMessage(), Color.WHITE);
        } catch (Exception e){
            mostrarMensagem(erroCadastro, "Erro ao salvar professor: " + e.getMessage(), Color.WHITE);
        }
    }
    ...
    void excluirProfessor(ActionEvent event) {
        ...
        if(selecionado == null)
            mostrarMensagem(erroTabela, "Escolha um professor para remover!", Color.RED);
        else {
