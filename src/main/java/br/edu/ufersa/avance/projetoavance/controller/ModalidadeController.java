...
public class ModalidadeController {
    ...
    private void imprimirPDF() {
        try {
            ...
            String titulo = "Relatório de Modalidades";
            List<String> cabecalhos = List.of("Nome", "Professor", "Tipo", "Vagas", "Valor", "Vagas Ocupadas");

            List<Map<String, String>> dados = modalidadeTable.getItems().stream()
                    .map(modalidade -> Map.of(
                            "Nome", modalidade.getNome(),
                            "Professor", modalidade.getProfessor() != null ? modalidade.getProfessor().getNome() : "N/A",
                            "Tipo", modalidade.getTipo().getDescricao(),
                            "Vagas", String.valueOf(modalidade.getVagas()),
                            "Valor", String.format("R$ %.2f", modalidade.getValor()),
                            "Vagas Ocupadas", modalidade.getVagasOcupadas() + "/" + modalidade.getVagas()
                    ))
                    .toList();
